...
var isPDFDownloadAjaxBased = false;
...
function ShowDatatableLoader(tableId) {
  if ($("#loader").length > 0) {
...
function HideDatatableLoader(tableId) {
...
function InitializeDataTables(
  dtColumns,
  dataUrl = "",
  enableButtonsParam = true,
  isAjaxBasedCrudParam = true,
  isResponsive = false,
  selectableRow = false,
  isExcelDownloadAjaxBasedParam = false,
  pageLength = 25,
  isPDFDownloadAjaxBasedParam = false
) {
  ...
  isPDFDownloadAjaxBased = isPDFDownloadAjaxBasedParam;
  ...
  //For Showing Loader
  $("#" + tableId).append(
    "<button id='loader' style='display:none' type='button' class='btn bg-custom-dark btn-float rounded-round'><i class='icon-spinner4 spinner'></i></button>"
  );

  $(window).off("scroll");
  $(window).on("scroll", function () {
    var isFixhedHeaderVisible = $(
  ...
  $(document).off(
    "keypress",
    "#filter-form input[type=search],#filter-form input[type=text]"
  );
  $(document).on(
    "keypress",
    "#filter-form input[type=search],#filter-form input[type=text]",
...
function FilterDataTable(
  ...
  dataTable = $("#" + tableId).DataTable({
    ...
    lengthMenu: [
      [10, 25, 50, 250, 1000, 5000, -1],
      [10, 25, 50, 250, 1000, 5000, "All"],
    ],
    //lBfrtipF
    dom:
      "<'datatable-header'B<'float-right'l>f>" + // Remove B to remove buttons
      "<'datatable-scroll'tr>" +
      "<'datatable-footer'ip>",
    ajax: {
      url: dataAjaxUrl,
      ...
      data: function (searchParams) {
        ...
        if ($("#" + formId).length > 0) {
          $("#" + formId + " :input, #" + formId + " select").each(function (
            ...
            if (val.value !== "") {
              ...
              } else searchParams[val.name] = $(val).val();
            }
          });
        }
        ...
        searchParams["CurrentPage"] =
          searchParams.start / searchParams.length + 1;
        ...
        if (searchParams.order.length > 0) {
          searchParams["OrderByColumn"] =
            dtColumns[searchParams.order[0].column].sortingColumn;
        ...
        render: function (data, type, row, meta) {
          if (dtColumns[meta.col].title !== "Action") {
            if (dtColumns[meta.col].format === "html") {
              if (
                type === "export" &&
                dtColumns[meta.col].exportColumn != null &&
                dtColumns[meta.col].exportColumn != ""
              ) {
              ...
              dtColumns[meta.col].title == "" ||
              dtColumns[meta.col].data == ""
            ) {
              return "";
            } else {
              return data;
    ...
    buttons: {
      dom: {
        button: {
          tag: "button",
          className:
            "btn rounded-round bg-custom-dark datatable-extension-button",
        },
      },
      buttons: [
        {
        ...
        },
        {
          ...
          action: function (e, dt, button, config) {
            if (isExcelDownloadAjaxBased) {
              ...
              var excelDataDownloaderUrl =
                "/" + controllerName + "/DownloadExcel";
              var selectedIds = [];
            ...
            ? [
                {
                  ...
                  action: function (e, dt, button, config) {
                    if (isPDFDownloadAjaxBased) {
                      var controllerName = window.location.href.split("/")[3];
                      var pdfDataDownloaderUrl =
                        "/" + controllerName + "/DownloadPDF";
                      var selectedIds = [];
                      ...
                      var pdfDataFilters = dt.ajax.params();
                      pdfDataFilters.SelectedIds = selectedIds;
                      pdfDataFilters.DisablePagination = true;

                      // Show loader
                      ShowDatatableLoader(tableId);

                      $.ajax({
                        url: pdfDataDownloaderUrl,
                        type: "POST",
                        data: pdfDataFilters,
                        xhrFields: {
                          responseType: "blob",
                        ...
                        success: function (data) {
                          ...
                          var link = document.createElement("a");
                          link.href = window.URL.createObjectURL(blob);
                          link.download = controllerName + " - Torrance.pdf";
                          document.body.appendChild(link);
                          link.click();
                          document.body.removeChild(link);
                        ...
                        error: function (xhr, textStatus, errorThrown) {
                          console.log(
                            "Error downloading PDF file: " + errorThrown
                          );
    ...
    drawCallback: function (settings) {
      ...
      $(".dataTables_scrollHead")
        ...
        .on("scroll", function (e) {
          ...
          if (
            !$(".dtfh-floatingparent.dtfh-floatingparenthead").is(":visible")
          ) {
            var scrollBody = $(this)
              .parent()
              .find(".dataTables_scrollBody")
...
function GetActionLinks(actionsList, cellData) {
  ...
  if (actionsList.length > 5) {
    ...
    $.each(actionsList, function (index, actionItem) {
      if (
        cellData.IsUserDefined === false ||
        cellData.IsUserDefined === undefined
      ) {
...
function GetHref(actionItem, cellData) {
  if (
    actionItem.HideBasedOn != null &&
    actionItem.HideBasedOn != undefined &&
    actionItem.HideBasedOn != ""
  ) {
    if (cellData[actionItem.HideBasedOn]) return "";
  }
  if (
    actionItem.ShowBasedOn != null &&
    actionItem.ShowBasedOn != undefined &&
    actionItem.ShowBasedOn != ""
  ) {
    if (
      cellData[actionItem.ShowBasedOn] == false ||
      cellData[actionItem.ShowBasedOn] == "false"
    )
      return "";
  }
  ...
  if (actionItem.Href !== "") {
    ...
    if (link.split("/").length > 3) {
      link =
        ...
        link.split("/")[2] +
        "/" +
        GetCellObjectValue(cellData, link.split("/")[3]);
    }
      ...
      if (linkAndQueryParams.length > 1) {
        ...
        $.each(queryParams, function (index, value) {
          link =
            ...
            $.trim(value.split("=")[0]) +
            "=" +
            GetCellObjectValue(cellData, $.trim(value.split("=")[0])) +
            "&";
        });
  ...
  if (actionItem.Attr !== null && actionItem.Attr.length > 0) {
    actionItem.Attr.forEach(function (v, i) {
      customAttributes +=
        ...
        v.toLowerCase() +
        '="' +
        GetCellObjectValue(cellData, v) +
        '" ';
    });
  }
  if (isAjaxBasedCrud && actionItem.Title != "Delete") {
    return `<a href="#" onclick="loadUpdateAndDetailModalPanel('${link}')" ${dataAttributes} class="datatable-action ${appendClass}" ${customAttributes}> <i class="${
      actionIcons[actionItem.Title]
    }"></i>${actionItem.LinkTitle}</a>`;
  }
  return (
    '<a href="' +
    link +
...
function RenderHtml(data, dtColumns, meta) {
  if (dtColumns[meta.col].formatValue === "checkbox") {
    return (
      '<input type="checkbox" class="checkbox-items ' +
      dtColumns[meta.col].className +
      '" value="' +
      data +
      '" />'
    );
    ...
    return (
      '<input type="text" class="text-box-items ' +
      dtColumns[meta.col].className +
      '" value="' +
      data +
      '" />'
    ...
    return (
      '<input type="number" class="number-text-box-items ' +
      dtColumns[meta.col].className +
      '" value="' +
      data +
      '" />'
    ...
    return (
      '<span data-barcode="' + data + '"><i class="fa fa-barcode"></i></span>'
    );
  } else if (dtColumns[meta.col].formatValue === "detail") {
    return (
      '<span class="details-control" data-url="' +
      dtColumns[meta.col].detailUrl +
      '">' +
      (data === undefined ? "" : data) +
      "</span>"
    );
    ...
    return (
      "<div>" +
      data +
      '</div><input type="hidden" class="hidden ' +
      dtColumns[meta.col].className +
      '" value="' +
      data +
      '">'
...
function RenderNumericValue(data, dtColumns, meta) {
  if (
    dtColumns[meta.col].formatValue !== null &&
    dtColumns[meta.col].formatValue !== ""
  ) {
...
function SetSearchFilters() {
  ...
  if (showSelectedFilters) {
    $(".search-filter-container input, .search-filter-container select").each(
      function (index) {
        ...
        if (input.attr("type") != "hidden" && input.val() != "") {
          ...
          containerHtml +=
            "<span class='badge badge-datatable-search mb-1' data-input-name='" +
            inputName +
            "'>" +
    ...
    if (containerHtml != "") {
      containerHtml +=
        "<span class='datatable-clear-container'><span class='badge badge-datatable-clear mb-1'>Clear</span></span>";
...
function RemoveSearchFilterInput(
  name,
  dataAjaxUrl,
  tableId,
  formId,
...
function DeleteItem(deleteObj) {
  ...
  $(document).on("click", "#validate-password", function () {
    ...
    if (password != null && password != "") {
      $.ajax({
        ...
        success: function (response) {
          if (response) {
            DeleteCardItem(deleteObj.deleteUrl).then(function (ajaxResult) {
              if (ajaxResult.Success) {
                ...
                if (ajaxResult.ReloadDatatable) {
                  SearchDataTable(
                    deleteObj.ajaxSearchUrl,
                    deleteObj.tableId,
                    deleteObj.formId,
                    deleteObj.actionsList,
                    deleteObj.dtColumns
                  );
                } else {
                  if (
                ...
                $("#crudDeleteModalError").html(
                  "<div class='alert alert-danger' role='alert'>Couldn't delete. Try again later.</div>"
                );
                $("#user-password").val("");
                $("#user-password").focus();
              }
            ...
            $("#crudDeleteModalError").html(
              "<div class='alert alert-danger' role='alert'>You've entered a wrong password.</div>"
            );
            $("#user-password").val("");
            $("#user-password").focus();
          }
      ...
      $("#crudDeleteModalError").html(
        "<div class='alert alert-danger' role='alert'>Please enter the password.</div>"
      );
    }
  });
}
...
function shouldColumnBeIgnoredForExport(element) {
  if ($(element).hasClass("include-in-export")) return false;
  else if (
    $(element).hasClass("exclude-from-export") ||
...
function customizePdfExport(doc) {
  ...
  if (colCount.length < 8)
    // Cuts columns for table with more than 8 columns
    doc.content[1].table.widths = colCount;
  var rowCount = dataTable.rows().count();
  for (i = 0; i < rowCount; i++) {
    for (j = 0; j < colCount.length; j++) {
      if (doc.content[1].table.body[i] && doc.content[1].table.body[i][j]) {
