...
namespace Repositories.Services.AppSettingServices.FCOLogService
{
        ...
        where UpdateViewModel : class, IBaseCrudViewModel, IIdentitifier, ISrNo, IFCOLogAttachment<AttachmentModifyViewModel>, new()
    {
        ...
        public async override Task<IRepositoryResponse> Create(CreateViewModel model)
        {
            using (var transaction = await _db.Database.BeginTransactionAsync())
            {
                try
                {
                    ...
                    string fcoIdentifier = await GetFCONumber((long)model.Unit.Id, model.SrNo);
        ...
        public async Task<XLWorkbook> DownloadExcel(FCOLogSearchViewModel searchModel)
        {
            try
            {
                ...
                for (var l = 0; l < logs.ReturnModel.Items.Count(); l++)
                {
                    ...
                    overrideLogSheet.Cell(rowNumber, 2).Value = logs.ReturnModel.Items[l].DateFormatted;
                    overrideLogSheet.Cell(rowNumber, 3).Value = logs.ReturnModel.Items[l].Unit?.Name ?? "-";
                    ...
                    overrideLogSheet.Cell(rowNumber, 15).Value = logs.ReturnModel.Items[l].Company?.Name ?? "-";
                    overrideLogSheet.Cell(rowNumber, 16).Value = logs.ReturnModel.Items[l].DesignatedCoordinator?.Name ?? "-";
                    overrideLogSheet.Cell(rowNumber, 17).Value = logs.ReturnModel.Items[l].ApprovalDateFormatted;
                    overrideLogSheet.Cell(rowNumber, 18).Value = logs.ReturnModel.Items[l].AreaExecutionLead?.Name ?? "-";
                    overrideLogSheet.Cell(rowNumber, 19).Value = logs.ReturnModel.Items[l].AreaExecutionLeadApprovalDate;
                    overrideLogSheet.Cell(rowNumber, 20).Value = logs.ReturnModel.Items[l].BusinessTeamLeader?.Name ?? "-";
                    overrideLogSheet.Cell(rowNumber, 21).Value = logs.ReturnModel.Items[l].BusinessTeamLeaderApprovalDate;
                    overrideLogSheet.Cell(rowNumber, 22).Value = logs.ReturnModel.Items[l].Rejecter?.Name ?? "-";
                    overrideLogSheet.Cell(rowNumber, 23).Value = logs.ReturnModel.Items[l].RejecterDate;
                    overrideLogSheet.Cell(rowNumber, 24).Value = logs.ReturnModel.Items[l].TotalCostFormatted;
        ...
        private void SetExcelHeaders(IXLWorksheet fcoLogSheet, long maxSectionRows)
        {
            ...
            fcoLogSheet.Cell(1, 2).Value = "Date";
            fcoLogSheet.Cell(1, 3).Value = "Unit";
            fcoLogSheet.Cell(1, 4).Value = "Service/Location";
            fcoLogSheet.Cell(1, 5).Value = "Pre TA/TA";
            ...
            fcoLogSheet.Cell(1, 8).Value = "Photo";
            fcoLogSheet.Cell(1, 9).Value = "Loop Identification # & Equipment Number";
            fcoLogSheet.Cell(1, 10).Value = "Description";
            fcoLogSheet.Cell(1, 11).Value = "File";
            fcoLogSheet.Cell(1, 12).Value = "FCO Type";
            fcoLogSheet.Cell(1, 13).Value = "Reason For Change";
            fcoLogSheet.Cell(1, 14).Value = "Other Documentation";
            fcoLogSheet.Cell(1, 15).Value = "Contractor";
            fcoLogSheet.Cell(1, 16).Value = "Designated Coordinator";
            fcoLogSheet.Cell(1, 17).Value = "Approval Date";
            //fcoLogSheet.Cell(1, 18).Value = "Company";
            //fcoLogSheet.Cell(1, 19).Value = "Employee";
            //fcoLogSheet.Cell(1, 20).Value = "Department";
            fcoLogSheet.Cell(1, 18).Value = "Area Execution Lead";
            fcoLogSheet.Cell(1, 19).Value = "Area Execution Lead Approval Date";
            fcoLogSheet.Cell(1, 20).Value = "Business Team Leader";
            fcoLogSheet.Cell(1, 21).Value = "Business Team Leader Approval Date";
            fcoLogSheet.Cell(1, 22).Value = "Rejecter";
            fcoLogSheet.Cell(1, 23).Value = "Rejecter Date";
            fcoLogSheet.Cell(1, 24).Value = "Total Cost";
            fcoLogSheet.Cell(1, 25).Value = "Total Hours";
            fcoLogSheet.Cell(1, 26).Value = "Total Head Count";
            fcoLogSheet.Cell(1, 27).Value = "Material Name";
            fcoLogSheet.Cell(1, 28).Value = "Material Rate";
            fcoLogSheet.Cell(1, 29).Value = "Equipment Name";
            fcoLogSheet.Cell(1, 30).Value = "Equipment Rate";
            fcoLogSheet.Cell(1, 31).Value = "Shop Name";
            fcoLogSheet.Cell(1, 32).Value = "Shop Rate";
